...
function loadDetail() {
  ...
  getListReport(getEventIdSelected(), onSuccessfulReport);
  ...
  $('#save-report').on('touchstart', function() {
    insertReport(currentEventID,$("txtContent").val(), imageCaptured, function(){
      getListReport(onSuccessfulReport);
    });
...
function onSuccessfulReport(listReport) {
  ...
  for (var i = 0; i < numberOfReport; i++) {
    ...
    var images = listReport[i]["images"];
    listReportItem += "<li class='table-view-cell media'><a class='navigate-right'><img class='media-object pull-left' src='"+images+"'><div class='media-body'><p>"+content+"</p></div></a></li>";
  }
  $("#list-report").html(listReportItem);
  // for (var i = 0; i < numberOfReport; i++) {
  //   var id = listReportItem[i]["ID"];
  //   document.getElementById(id).addEventListener("touchstart", function() {
  //     setEventIdSlected(this.id);
  //   });
  // }
